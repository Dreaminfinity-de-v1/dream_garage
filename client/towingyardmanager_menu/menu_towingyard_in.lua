function addTowingyardinMenu(menu)

    local garage = getGarageFromId(interactionArea)

    getTowingyardVehiclesInParkingarea(function(vehicles)
        if #vehicles > 0 then
            ...
            menu:AddItem(NativeUI.CreateItem(_U('towingyard_parkingin_noitem'), _U('towingyard_parkingin_noitem_desc')))
        end
    ...
    if GetVehiclePedIsIn(ped,false) ~= data.id then
        ...
        getTowingyardVehiclesInParkingarea(function(vehicle)
            ...
            if vehicle ~= nil then
                ESX.TriggerServerCallback('dream_garage:setVehicleInparking', function(error)
                    ...
                    if error == 'ok' then
                        if deleteTowingyardVehicle(data, menu, index) == true then
                            TriggerEvent("swt_notifications:captionIcon",_U('notifications_towingyard_titel'),_U('notification_message_parkingin'),
                    ...
                    elseif error == 'already_in' then
                        if deleteVehicle(data, menu, index) == true then
                            TriggerEvent("swt_notifications:captionIcon",_U('notifications_towingyard_titel'),_U('notification_message_parkingin_already_in'),
                ...
                TriggerEvent("swt_notifications:captionIcon",_U('notifications_towingyard_titel'),_U('notification_message_parkingin_not-in-area'),
                    Config.Notification.pos,Config.Notification.timeout,Config.Notification.color.negative,'white',true,Config.Notification.icons.garage_warn)
            end
        end, garage.vehicle_types, data.data.plate)
    else
        TriggerEvent("swt_notifications:captionIcon",_U('notifications_towingyard_titel'),_U('notification_message_parkingin_in-the-vehicle'),
...
function deleteTowingyardVehicle(data, menu, index)
    ...
    while not NetworkHasControlOfEntity(data.id) and attempt < 20 and DoesEntityExist(data.id) do
        ...
        if attempt % 10 == 0 then
            TriggerEvent("swt_notifications:captionIcon",_U('notifications_towingyard_titel'),_U('notification_message_parking_attempt-info'),
                Config.Notification.pos,1,Config.Notification.color.wait,'white',true,Config.Notification.icons.car_wait)
        end
        attempt = attempt + 1
    end
